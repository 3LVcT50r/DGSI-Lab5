...
def seed_database_from_config(
    ...
    """Seed the DB with parts, BOM, suppliers and state."""
    ...
    material_names = set()
    for model_name, model_info in config_data.get(
        "models", {}
    ).items():
    ...
    for mat_name in material_names:
        mat_prod = Product(
            name=mat_name, type=ProductType.RAW
        )
    ...
    for model_name, model_info in config_data.get(
        ...
        fin_prod = Product(
            ...
            wholesale_price=model_info.get("wholesale_price", 150.0),
        )
    ...
    for model_name, model_info in config_data.get(
        ...
        for mat_name, qty in model_info.get(
            "bom", {}
        ).items():
            if not isinstance(qty, (int, float)):
                logger.info(
                    "Skipping non-numeric BOM: "
                    "%s = %s", mat_name, qty,
                )
                continue
        ...
        for prod_info in supp_info.get("products", []):
            ...
            if product_name in products_cache:
                supplier = Supplier(
                    name=supp_info["name"],
                    product_id=(
                        products_cache[product_name].id
                    ),
                    unit_cost=prod_info[
                        "price_per_unit"
                    ],
                    lead_time_days=prod_info[
                        "lead_time_days"
                    ],
                    min_order_qty=prod_info.get(
                        "min_order_qty", 1
                    ),
                )
